...
- **Phase E — Integrations**: IN PROGRESS. File import engine (CSV + Excel .xlsx/.xls) via `server/services/file-parser.ts` using `xlsx` package. Admin imports page updated to accept both formats. New tenant-level `POST /api/tenants/:tenantId/import-file` endpoint for client-accessible imports (not admin-only). Enter Data page has "Import File" button with dialog showing expected format, auto-column detection (metric/kpi/name + value/amount/actual), and inline error reporting. Future: Connect to third-party apps via API (QuickBooks, Xero, Square, Toast, Stripe, Google Sheets, Zapier, etc.). Per-tenant API key/token storage. Scheduled data sync.
